...
app.post('/api/register', registerLimiter, async (req, res) => {
  const { id, name, bday, password, securityQuestion, securityAnswer, grade, classroom } = req.body;
  ...
  const { error } = await supabase.from('users').insert({
    ...
    securityQuestion: securityQuestion||null, securityAnswer: securityAnswer||null,
...
app.post('/api/verify-security', async (req, res) => {
  ...
  if (!user.securityAnswer || user.securityAnswer.trim().toLowerCase() !== (securityAnswer||'').trim().toLowerCase())
    return res.status(403).json({ error: '보안 질문 답변이 틀렸어요.' });
  res.json({ ok: true });
});
...
app.get('/api/profile', auth, async (req, res) => {
  const { data } = await supabase.from('users').select('id,name,role,email,email_verified,grade,classroom,avatar,bday,warnings').eq('id', req.user.id).single();
  res.json(data||{});
});
...
app.post('/api/auth/send-verify', async (req, res) => {
  ...
  await supabase.from('users').update({ email, verify_code: code, verify_expires: expires, email_verified: false }).eq('id', userId);
  try {
...
app.post('/api/auth/verify-email', async (req, res) => {
  ...
  const { data: user } = await supabase.from('users').select('verify_code,verify_expires').eq('id', userId).single();
  if (!user) return res.status(404).json({ error: '사용자를 찾을 수 없어요.' });
  if (!user.verify_code || user.verify_code !== code) return res.status(400).json({ error: '인증 코드가 틀렸어요.' });
  if (new Date() > new Date(user.verify_expires)) return res.status(400).json({ error: '인증 코드가 만료됐어요. 다시 발송해주세요.' });
  await supabase.from('users').update({ email_verified: true, verify_code: null }).eq('id', userId);
  res.json({ ok: true });
});
...
app.post('/api/attendance', auth, checkSuspended, async (req, res) => {
  ...
  res.json({ ok: true, total: count || 1 });
});
...
app.get('/api/attendance', auth, async (req, res) => {
  ...
  res.json({ total: count || 0, checkedToday: !!todayData });
});
...
app.get('/api/auth/kakao/callback', async (req, res) => {
  ...
  try {
    ...
    const userData = encodeURIComponent(JSON.stringify({ id: user.id, name: user.name, role: user.role, bday: user.bday, warnings: user.warnings, grade: user.grade, classroom: user.classroom }));
