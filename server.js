...
app.post('/api/posts/:id/like', auth, checkSuspended, async (req, res) => {
  ...
  if (ex) { await supabase.from('likes').delete().eq('post_id',pid).eq('uid',uid); await supabase.rpc('decrement_likes',{post_id:parseInt(pid)}); return res.json({liked:false}); }
  await supabase.from('dislikes').delete().eq('post_id',pid).eq('uid',uid);
  await supabase.from('likes').insert({post_id:pid,uid});
  await supabase.rpc('increment_likes',{post_id:parseInt(pid)});
  await supabase.rpc('decrement_dislikes',{post_id:parseInt(pid)});
...
app.post('/api/posts/:id/dislike', auth, checkSuspended, async (req, res) => {
  ...
  if (ex) { await supabase.from('dislikes').delete().eq('post_id',pid).eq('uid',uid); await supabase.rpc('decrement_dislikes',{post_id:parseInt(pid)}); return res.json({disliked:false}); }
  await supabase.from('likes').delete().eq('post_id',pid).eq('uid',uid);
  await supabase.from('dislikes').insert({post_id:pid,uid});
  await supabase.rpc('increment_dislikes',{post_id:parseInt(pid)});
  await supabase.rpc('decrement_likes',{post_id:parseInt(pid)});
