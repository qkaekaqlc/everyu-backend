...
app.get('/api/posts', auth, async (req, res) => {
  ...
  let query = supabase.from('posts').select('id,uid,author,anon,cat,title,body,imgs,files,likes,dislikes,deleted,pinned,edited,created_at,views').eq('deleted', false).order('pinned',{ascending:false}).order('created_at',{ascending:false}).range((page-1)*limit, page*limit-1);
  ...
  res.json((data||[]).map(p=>({...p,comment_count:cm[p.id]||0})));
});
...
app.post('/api/posts/:id/like', auth, checkSuspended, async (req, res) => {
  ...
  if (ex) { await supabase.from('likes').delete().eq('post_id',pid).eq('uid',uid); await supabase.from('posts').update({likes:supabase.raw('GREATEST(likes-1,0)')}).eq('id',pid); return res.json({liked:false}); }
  await supabase.from('dislikes').delete().eq('post_id',pid).eq('uid',uid);
  await supabase.from('likes').insert({post_id:pid,uid});
  await supabase.from('posts').update({likes:supabase.raw('likes+1'),dislikes:supabase.raw('GREATEST(dislikes-1,0)')}).eq('id',pid);
  const { data: p } = await supabase.from('posts').select('uid,title').eq('id',pid).single();
...
app.post('/api/posts/:id/dislike', auth, checkSuspended, async (req, res) => {
  ...
  if (ex) { await supabase.from('dislikes').delete().eq('post_id',pid).eq('uid',uid); await supabase.from('posts').update({dislikes:supabase.raw('GREATEST(dislikes-1,0)')}).eq('id',pid); return res.json({disliked:false}); }
  await supabase.from('likes').delete().eq('post_id',pid).eq('uid',uid);
  await supabase.from('dislikes').insert({post_id:pid,uid});
  await supabase.from('posts').update({dislikes:supabase.raw('dislikes+1'),likes:supabase.raw('GREATEST(likes-1,0)')}).eq('id',pid);
...
app.post('/api/posts/:id/comments', auth, checkSuspended, commentLimiter, async (req, res) => {
  ...
  const { data } = await supabase.from('comments').insert({ post_id:req.params.id, uid:req.user.id, author:req.user.name, body:filterBadWords(sanBody(body)), anon:true, parent_id:parentId||null, likes:0, dislikes:0 }).select().single();
  const { data: post } = await supabase.from('posts').select('uid,title').eq('id',req.params.id).single();
  if (post&&post.uid!==req.user.id) await supabase.from('notifications').insert({to_uid:post.uid,text:'회원님의 글 "'+post.title+'"에 댓글이 달렸어요. 💬',post_id:req.params.id,read:false});
  if (parentId) { const { data: pc } = await supabase.from('comments').select('uid').eq('id',parentId).single(); if (pc&&pc.uid!==req.user.id) await supabase.from('notifications').insert({to_uid:pc.uid,text:'회원님의 댓글에 답글이 달렸어요. ↩',post_id:req.params.id,read:false}); }
  res.json(data);
});
...
app.post('/api/comments/:id/like', auth, checkSuspended, async (req, res) => {
  ...
  if (ex) { await supabase.from('comment_likes').delete().eq('comment_id',cid).eq('uid',uid); await supabase.from('comments').update({likes:supabase.raw('GREATEST(likes-1,0)')}).eq('id',cid); return res.json({liked:false}); }
  ...
  await supabase.from('comments').update({likes:supabase.raw('likes+1'),dislikes:supabase.raw('GREATEST(dislikes-1,0)')}).eq('id',cid);
  res.json({liked:true});
});
...
app.post('/api/comments/:id/dislike', auth, checkSuspended, async (req, res) => {
  ...
  if (ex) { await supabase.from('comment_dislikes').delete().eq('comment_id',cid).eq('uid',uid); await supabase.from('comments').update({dislikes:supabase.raw('GREATEST(dislikes-1,0)')}).eq('id',cid); return res.json({disliked:false}); }
  ...
  await supabase.from('comments').update({dislikes:supabase.raw('dislikes+1'),likes:supabase.raw('GREATEST(likes-1,0)')}).eq('id',cid);
  res.json({disliked:true});
});
...
app.listen(PORT, () => console.log(`🚀 에브리유니 서버 실행 중 → 포트 ${PORT}`));
