...
app.post('/api/schools', auth, adminOnly, async (req, res) => {
  ...
  // nx, ny 자동 계산
  let calcNx = nx, calcNy = ny;
  if (!nx && lat && lon) {
    const { nx: cx, ny: cy } = latLonToGrid(parseFloat(lat), parseFloat(lon));
    calcNx = cx; calcNy = cy;
  }
  const { data, error } = await supabase.from('schools').insert({ name, office_code, school_code, address, lat, lon, nx: calcNx, ny: calcNy, bus_stops: bus_stops || [] }).select().single();
