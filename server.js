...
app.post('/api/register', registerLimiter, async (req, res) => {
  const { id, name, bday, password, securityQuestion, securityAnswer, grade, classroom, email } = req.body;
...
app.post('/api/login', loginLimiter, async (req, res) => {
  ...
  if (!user.password) return res.status(401).json({ error: '카카오로 가입된 계정이에요. 카카오로 로그인해주세요.' });
...
app.delete('/api/account', auth, async (req, res) => {
  ...
  const { data: user } = await supabase.from('users').select('password,kakao_id').eq('id', req.user.id).single();
  // 카카오 유저가 아닌 경우 비밀번호 확인
  if (!user?.kakao_id) {
...
// ══ NEIS 급식/시간표 ══
const NEIS_KEY = 'a4189b4517b84ad593b912a5c8362bf5';
...
app.post('/api/reports', auth, checkSuspended, async (req, res) => {
  ...
  // 중복 신고 방지
  const dupQuery = supabase.from('reports').select('id').eq('reporter_uid', req.user.id).eq('resolved', false);
  if (pid) dupQuery.eq('post_id', pid);
  if (cid) dupQuery.eq('comment_id', cid);
  const { data: dupReport } = await dupQuery.single();
...
// ══ 주간 인기글 ══
app.get('/api/posts/weekly', auth, async (req, res) => {
...
app.post('/api/posts/:id/reactions', auth, checkSuspended, async (req, res) => {
  ...
  const { data: ex } = await supabase.from('reactions').select('id').eq('post_id',pid).eq('uid',uid).eq('emoji',emoji).single();
  if (ex) {
...
app.post('/api/admin/users/:id/warn', auth, adminOnly, async (req, res) => {
  ...
  await supabase.from('logs').insert({uid:req.user.id,action:user.name+' 경고 부여 ('+nw+'회) (관리자: '+req.user.id+')',type:'warn'});
  ...
  if (fullReport?.post_id) {
    ...
    if (post?.uid) {
      ...
      if (resolvedCount && [3,5,7].includes(resolvedCount)) {
        ...
        const upd = {warnings:nw}; if(nw>=3) upd.suspended=true;
        await supabase.from('users').update(upd).eq('id', post.uid);
        await supabase.from('notifications').insert({to_uid:post.uid,text:`⚠️ 신고 누적으로 경고가 부여됐어요. (누적 ${nw}회)`,read:false});
        if(nw>=3) await supabase.from('notifications').insert({to_uid:post.uid,text:'🚫 경고 3회 누적으로 계정이 정지됐어요.',read:false});
      }
    }
  }
...
app.post('/api/upload', auth, checkSuspended, async (req, res) => {
  ...
  // 허용된 확장자만 업로드 가능
  const ALLOWED_EXTS = ['jpg','jpeg','png','gif','webp','pdf','docx','xlsx','pptx','txt','zip'];
  const BLOCKED_EXTS = ['html','htm','js','ts','php','py','exe','bat','sh','cmd','apk','msi','svg'];
...
// ══ 카카오 로그인 ══
const KAKAO_REST_KEY = process.env.KAKAO_REST_KEY || 'd672dda8d7071213aca221cb1d4fc32c';
