...
app.get('/api/schedule', auth, async (req, res) => {
  ...
  try {
    const url = `https://open.neis.go.kr/hub/SchoolSchedule?KEY=${NEIS_KEY}&Type=json&ATPT_OFCDC_SC_CODE=${OFFICE_CODE}&SD_SCHUL_CODE=${SCHOOL_CODE}&AA_FROM_YMD=${from}&AA_TO_YMD=${to}`;
    const resp = await fetch(url);
    const data = await resp.json();
    if (data.SchoolSchedule) {
      const rows = data.SchoolSchedule[1].row;
      res.json(rows.map(r => ({ date: r.AA_YMD, name: r.EVENT_NM })));
    } else {
      res.json([]);
    }
  } catch(e) {
    res.status(500).json({ error: '학사일정을 불러오지 못했어요.' });
  }
});
