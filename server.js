...
app.post('/api/rankings', auth, async (req, res) => {
  ...
  await supabase.from('game_rankings').insert({ uid: req.user.id, name: req.user.name, game, score });
...
app.get('/api/rankings/:game', auth, async (req, res) => {
  ...
  const { data } = await supabase.from('game_rankings').select('name,score,created_at').eq('game', req.params.game).order('score', { ascending: lowerIsBetter }).limit(10);
  res.json(data || []);
});
